...
def validate_password(field):
    ...
    if not 8 <= len(field) <= 16:
        ...
        raise ValidationError('The password must contain from 8 to 16 symbols.', code='invalid')
    if not bool(re.match(pattern, field)):
        print('The password must contain only digits and Latin letters.')
        raise ValidationError('The password must contain only digits and Latin letters.', code='invalid')